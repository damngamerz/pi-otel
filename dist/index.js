...
export default function piOtel(pi) {
    ...
    registerStatusCommand(pi, () => config, () => runtime);
    registerFlushCommand(pi, () => runtime);
    const evaluateLatest = async (ctx, force) => {
        if (!config || !runtime || evaluationInFlight)
            return;
        if (config.evaluation.mode === "off") {
            if (force) {
                ctx.ui.notify("Remote evaluation is disabled. Set piOtel.evaluation.mode to manual, sample, or always.", "warning");
            }
            return;
        }
        ...
        if (!pair) {
            if (force)
                ctx.ui.notify("No completed user/assistant pair is available to evaluate.", "warning");
            return;
        }
        const exchangeHash = anonymize(`${pair.userRequest}\0${pair.assistantResponse}`);
        if (!shouldEvaluate(config.evaluation.mode, config.evaluation.sampleRate, exchangeHash, force))
            return;
        if (evaluatedExchangeHashes.has(exchangeHash)) {
            if (force)
                ctx.ui.notify("The latest exchange has already been evaluated.", "info");
            return;
        }
        if (config.evaluation.blockLikelySecrets &&
            (containsLikelySecret(pair.userRequest) || containsLikelySecret(pair.assistantResponse))) {
            ctx.ui.notify("Remote evaluation skipped: the latest exchange appears to contain a credential or private key.", "warning");
            return;
        }
        ...
        try {
            ...
            ctx.ui.notify([
                ...
                ...evaluation.result.scores.map((score) => `  ${score.name}: ${score.score.toFixed(2)} (${score.label})`),
            ...
            ctx.ui.notify(`Remote evaluation failed: ${error instanceof Error ? error.message : String(error)}`, "error");
        }
        finally {
    ...
    pi.on("session_start", async (_event, ctx) => {
        try {
            ...
            providers = undefined;
            runtime = undefined;
            ctx.ui.notify(`Pi OTel disabled: ${error instanceof ConfigError ? error.message : String(error)}`, "error");
        }
    });
    ...
    pi.on("message_update", async (event) => {
        if (isAssistantStreamUpdate(event.message, event.assistantMessageEvent))
            runtime?.noteFirstToken();
    });
    pi.on("message_end", async (event) => {
        if (isAssistantMessage(event.message))
            runtime?.finishLlm(event.message);
    });
    ...
    pi.on("session_shutdown", async (_event, ctx) => {
        try {
            ...
            ctx.ui.notify(`Pi OpenTelemetry shutdown failed: ${error instanceof Error ? error.message : String(error)}`, "error");
        }
        if (ctx.hasUI) {
