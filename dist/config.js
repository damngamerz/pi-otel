...
function assertAllowedKeys(value, allowed, path) {
    ...
    if (unknown)
        throw new ConfigError(`${path}.${unknown} is not a supported setting`);
}
...
function validateHeaders(value) {
    if (!isRecord(value))
        throw new ConfigError("piOtel.headers must be an object of string values");
    const headers = {};
...
export function validateSettingsFile(value, source = "settings") {
    if (value == null)
        return undefined;
    if (!isRecord(value))
        throw new ConfigError(`${source} must contain a JSON object`);
    if (value.piOtel == null)
        return {};
    ...
    if (raw.endpoint !== undefined)
        parsed.endpoint = requireString(raw.endpoint, "piOtel.endpoint");
    ...
    if (raw.signals !== undefined) {
        if (!isRecord(raw.signals))
            throw new ConfigError("piOtel.signals must be an object");
    ...
    if (raw.evaluation !== undefined) {
        if (!isRecord(raw.evaluation))
            throw new ConfigError("piOtel.evaluation must be an object");
        assertAllowedKeys(raw.evaluation, ["mode", "sampleRate", "provider", "model", "maxCharsPerField", "blockLikelySecrets"], "piOtel.evaluation");
        const evaluation = {};
        if (raw.evaluation.mode !== undefined)
            evaluation.mode = normalizeMode(raw.evaluation.mode);
        ...
        if (raw.evaluation.maxCharsPerField !== undefined) {
            evaluation.maxCharsPerField = requireNumber(raw.evaluation.maxCharsPerField, "piOtel.evaluation.maxCharsPerField");
        }
        if (raw.evaluation.blockLikelySecrets !== undefined) {
            evaluation.blockLikelySecrets = requireBoolean(raw.evaluation.blockLikelySecrets, "piOtel.evaluation.blockLikelySecrets");
        }
...
export function validateEndpoint(endpoint, allowRemoteEndpoint) {
    let url;
    try {
...
export function resolveConfigFromSources(globalSettingsInput, projectSettingsInput, env = process.env) {
    ...
    return {
        ...
        evaluation: {
            ...
            blockLikelySecrets: envBoolean(env.PI_OTEL_EVALUATION_BLOCK_SECRETS) ?? config.evaluation.blockLikelySecrets,
        },
    };
}
