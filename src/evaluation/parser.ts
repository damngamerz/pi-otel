...
import type { EvaluationResult, EvaluationScore } from "./types.js";

const REQUIRED_NAMES = ["task_success", "instruction_following", "relevance", "correctness"] as const;
const VALID_LABELS = new Set(["poor", "fair", "good", "excellent"]);

export function parseJudgeResult(text: string): EvaluationResult {
	const fenced = text.match(/```(?:json)?\s*([\s\S]*?)```/i)?.[1];
	const firstBrace = text.indexOf("{");
	const lastBrace = text.lastIndexOf("}");
	const candidateText =
		fenced ?? (firstBrace >= 0 && lastBrace > firstBrace ? text.slice(firstBrace, lastBrace + 1) : text);
	let parsed: {
		scores?: Array<{ name?: unknown; score?: unknown; label?: unknown }>;
		summary?: unknown;
		issues?: unknown;
	};
	try {
	...
	const scores: EvaluationScore[] = (parsed.scores ?? []).map((entry) => {
	...
	if (
		scores.length !== REQUIRED_NAMES.length ||
	...
	const orderedScores = REQUIRED_NAMES.map((name) => {
		...
		if (!score) throw new Error(`Judge omitted required score: ${name}`);
		return score;
	});
	return {
